...
import com.hapi.ioutput.muxer.internal.utils.av.video.avc.AVCDecoderConfigurationRecord
import com.hapi.ioutput.muxer.internal.utils.av.video.hevc.HEVCDecoderConfigurationRecord
import com.hapi.ioutput.muxer.internal.utils.av.video.getStartCodeSize
import com.hapi.ioutput.muxer.internal.utils.av.video.removeStartCode
...
class VideoTag(
    ...
    init {
        if (videoConfig.getString(MediaFormat.KEY_MIME) == MediaFormat.MIMETYPE_VIDEO_AVC) {
            if (!isSequenceHeader) {
        ...
        if (videoConfig.getString(MediaFormat.KEY_MIME) == MediaFormat.MIMETYPE_VIDEO_HEVC) {
            if (!isSequenceHeader) {
                require(buffers.size == 1) { "Only one buffer is expected for raw frame" }
            } else {
    ...
    override fun writeTagHeader(buffer: ByteBuffer) {
        ...
        buffer.put(
            (frameType.value shl 4) or // Frame Type
                    (CodecID.fromMimeType(videoConfig.getString(MediaFormat.KEY_MIME)!!).value) // CodecId
        )
        if ((videoConfig.getString(MediaFormat.KEY_MIME) == MediaFormat.MIMETYPE_VIDEO_AVC)
            || (videoConfig.getString(MediaFormat.KEY_MIME) == MediaFormat.MIMETYPE_VIDEO_HEVC)
        ) {
            if (isSequenceHeader) {
    ...
    override fun writePayload(buffer: ByteBuffer) {
        if (isSequenceHeader) {
            when (videoConfig.getString(MediaFormat.KEY_MIME)) {
                ...
                MediaFormat.MIMETYPE_VIDEO_HEVC -> {
                    ...
                    throw IOException("Mimetype ${videoConfig.getString(MediaFormat.KEY_MIME)} is not supported")
                }
            }
