...
import com.hapi.ioutput.muxer.internal.muxers.ts.data.ITSElement
...
import com.hapi.ioutput.muxer.internal.utils.putShort
...
class Pmt(
    ...
    private val programInfoLength: Int
        get() = getProgramInfoLength(MediaFormat.MIMETYPE_AUDIO_OPUS) * streams.filter { it.config.getString(MediaFormat.KEY_MIME) == MediaFormat.MIMETYPE_AUDIO_OPUS }.size + getProgramInfoLength(
            MediaFormat.MIMETYPE_VIDEO_HEVC
        ) * streams.filter { it.config.getString(MediaFormat.KEY_MIME) == MediaFormat.MIMETYPE_VIDEO_HEVC }.size
    ...
    override fun toByteBuffer(): ByteBuffer {
        ...
        streams.forEach {
            buffer.put(StreamType.fromMimeType(it.config.getString(MediaFormat.KEY_MIME)!!).value)
            buffer.putShort(
                (0b111 shl 13) // Reserved
                        or (it.pid.toInt())
            )
            buffer.putShort(
                (0b1111 shl 12) // Reserved
                        or getProgramInfoLength(it.config.getString(MediaFormat.KEY_MIME)!!)
            ) // First two bits of ES_info_length shall be '00' + ES_info_length

            // Registration descriptor
            if (it.config.getString(MediaFormat.KEY_MIME) == MediaFormat.MIMETYPE_AUDIO_OPUS) {
                putRegistrationDescriptor(buffer, "Opus")
                buffer.put(0x7F)
                buffer.put(0x02)
                buffer.put(0x80.toByte())
               //TODO buffer.put(AudioConfig.getNumberOfChannels((it.config as AudioConfig).channelConfig))
            } else if (it.config.getString(MediaFormat.KEY_MIME) == MediaFormat.MIMETYPE_VIDEO_HEVC) {
