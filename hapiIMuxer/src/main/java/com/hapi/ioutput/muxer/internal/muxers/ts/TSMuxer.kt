...
import com.hapi.ioutput.muxer.internal.muxers.ts.packets.Pat
import com.hapi.ioutput.muxer.internal.muxers.ts.packets.Pes
import com.hapi.ioutput.muxer.internal.muxers.ts.packets.Pmt
import com.hapi.ioutput.muxer.internal.muxers.ts.packets.Sdt
import com.hapi.ioutput.muxer.internal.muxers.ts.utils.MuxerConst
import com.hapi.ioutput.muxer.internal.muxers.ts.utils.TSConst
import com.hapi.ioutput.muxer.internal.utils.av.audio.aac.ADTS
...
class TSMuxer(
    ...
    initialStreams: List<MediaFormat>? = null,
) : IMuxer {
     ...
     */
    override fun addStreams(streamsConfig: List<MediaFormat>) =
        addStreams(getServices()[0], streamsConfig)

    override fun removeStreams(streamsPid: List<Int>) =
        removeStreams(getServices()[0], streamsPid.map {
            it.toShort()
        })
     ...
     * @return ordered list of stream id
     */
    private fun addStreams(tsServiceInfo: TsServiceInfo, streamsConfig: List<MediaFormat>) =
    ...
    private fun addStreams(
        service: Service,
        streamsConfig: List<MediaFormat>
    ): Map<MediaFormat, Int> {
        require(tsServices.contains(service)) { "Unknown service" }
        ...
        service.pcrPid = try {
            service.streams.first { it.config.getString(MediaFormat.KEY_MIME)!!.isVideo() }.pid
     ...
     */
    private fun removeStreams(tsServiceInfo: TsServiceInfo, streamsPid: List<Short>) =
...
//        }
    ...
    fun getStreams(mimeType: String): List<Stream> {
        return tsServices.flatMap { it.streams }
            .filter { it.config.getString(MediaFormat.KEY_MIME) == mimeType }
    }
